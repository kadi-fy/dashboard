      ...
      >
        ...
        >
          ...
          <div class="p-6 overflow-y-auto max-h-[80vh]">
            <div class="mb-6">
              <h4 class="text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">月度营业收入趋势（万元）</h4>
              <BarChart
...
const annualPlan = computed(() =>
  currentYearData.value.length > 0 ? currentYearData.value[0].annual_plan_internal_revenue : 0
)
...
const currentYearMap = computed(() => {
  ...
  currentYearData.value.forEach(item => {
    map.set(item.time_id % 100, item.revenue_total)
  })
  return map
})
