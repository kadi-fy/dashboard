    ...
    >
      ...
      >
        ...
        <div class="p-6 overflow-y-auto max-h-[80vh]">
          ...
          <div class="mb-6">
            <h4 class="text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">净合同合计趋势（万元）</h4>
...
const charges = computed(() =>
  props.currentYearData.map(item => (item.contract_total)) 
)

// 去年数据通过 Map 对齐到今年的月份
const lastYearCharges = computed(() => {
  const map = new Map(props.lastYearData.map(item => [item.time_id % 100, item.contract_total]))
...
const summary = computed(() => {
  ...
  return {
    annualPlan: last.contract_plan,
    totalNetCharge: last.contract_total,
    completionRate: (last.contract_total / last.contract_plan) * 100
  }
})
    ...
    data.datasets.forEach((dataset, datasetIndex) => {
      ...
      meta.data.forEach((element, index) => {
        ...
        if (dataset.label?.includes('实际完成')) {
          ...
          const percentage = plan ? ((currentItem?.contract_total || 0) / plan) * 100 : 0
          ...
          callbacks: {
            label: (context) => {
              ...
              if (label.includes('实际完成')) {
                ...
                const pct = plan ? ((currentItem?.contract_total || 0) / plan) * 100 : 0
